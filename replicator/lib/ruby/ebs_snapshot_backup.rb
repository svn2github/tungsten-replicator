...
class EBSSnapshotBackup < TungstenSnapshotBackup
  def create_snapshot(timestamp)
    ...
    ds.snapshot_paths.each{
      ...
      if mount_point_ids.has_key?(mount_point)
        ...
        # work in Ruby 1.8
        volume_type = TU.cmd_result(". /etc/profile.d/aws-apitools-common.sh; /opt/aws/bin/ec2-describe-volumes #{volume.id} --show-empty-fields --hide-tags | grep VOLUME | awk -F' ' '{print $8}'")
  ...
  def restore_snapshot(snapshot_ids)
    ...
    begin
      ...
      remove_volume_attachments(mount_point_original_attachments)
      unless TU.is_valid?()
        return false
      end
  ...
  def remove_volume_attachments(attachments)
    attachments.each{
      |mount_point, attachment|
