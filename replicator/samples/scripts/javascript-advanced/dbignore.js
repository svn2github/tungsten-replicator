 ...
 * Copyright (C) 2012-2012 Continuent Inc.
 * Contact: tungsten@continuent.org
 *
 * JavaScript example for JavaScriptFilter. Allows one to select a specific
 * schema to replicate while ignoring others on the slave. This way one
 * can configure multiple slaves to replicate different schemas or just ignore
 * the ones don't needed. Works on both statement and row replication.
 *
 * Example of how to define one in replicator.properties:
 *
 * replicator.filter.dbignore=com.continuent.tungsten.replicator.filter.JavaScriptFilter
 * replicator.filter.dbignore.script=../samples/extensions/javascript/dbignore.js
 * replicator.filter.dbignore.db=dbtoignore
 */
...
function prepare()
{
    ...
    logger.info("dbignore: Ignoring statements from schema \"" + schemaName + "\"");
}
...
function filter(event)
{
    ...
    for(i=0;i<data.size();i++)
    {
        ...
        if(d instanceof com.continuent.tungsten.replicator.dbms.StatementData)
        {
            ...
            if(d.getDefaultSchema()!=null && d.getDefaultSchema().compareTo(db)==0)
            {
                // Our DB - ignore the statement!
                data.remove(i);
                logger.debug("dbignore: Removed statement: \"" + d.getQuery() + "\"");
                logger.debug("dbignore: Came from default schema: \"" + d.getDefaultSchema() + "\"");
        ...
        else if(d instanceof com.continuent.tungsten.replicator.dbms.RowChangeData)
        {
            ...
            for(j=0;j<rowChanges.size();j++)
            {
                ...
                if(oneRowChange.getSchemaName()!=null && oneRowChange.getSchemaName().compareTo(db)==0)
                {
                    ...
                    logger.debug("dbignore: Removed row operation: table = \"" + schemaName + "."
                             + tableName + "\"");
                    // As we removed the array element all other ones came closer to us:
                    j--;
                }
