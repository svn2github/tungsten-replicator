...
public class ResultFormatter
{
    ...
    {
        String[] columnNames = properties.map().keySet().toArray(
                (new String[properties.map().size()]));
        Vector<String[]> results = new Vector<String[]>();
        results.add(properties.map().values().toArray(
                new String[properties.map().size()]));
            ...
            int indentToUse, boolean ignoreColon, boolean useDelimiters)
    {
        ...
        for (int i = 0; i < entries.length; i++)
        {
            ...
            for (String element : elements)
            {
                String[] subElements = ResultFormatter.wrap(element.replace(
                        "\t", "  "), entryWidth - 2, indentToUse, ignoreColon);
